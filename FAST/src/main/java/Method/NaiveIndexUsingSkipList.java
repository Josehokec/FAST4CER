...
public class NaiveIndexUsingSkipList extends Index {
    ...
    public NaiveIndexUsingSkipList(String indexName){
        super(indexName);
    }
    ...
    public int countQuery(EventPattern pattern, AbstractJoinStrategy join){
        ...
        for(int i = 0; i < patternLen; ++i){
            ...
            // Putting all RIDs together is faster than accessing them individually
            // because it satisfies the sequential access property
            List<IthRID> curIthRID = getRIDList(pairs, i);
            queryRIDList = mergeRID(queryRIDList, curIthRID);

            alphas.add(selPairs);
        }
        // because use some skills to optimized algorithm, evaluate sum cost has some error
    ...
    public List<Tuple> tupleQuery(EventPattern pattern, AbstractJoinStrategy join){
        ...
        for(int i = 0; i < patternLen; ++i){
            ...
            List<IndependentConstraint> icList = pattern.getICListUsingVarName(varName);
            // 满足独立谓词的事件，已经排序
            List<IndexValuePair> triples = getAllPairs(eventType, icList);
            List<byte[]> recordList = getRecordList(triples);
            buckets.add(recordList);
        }
